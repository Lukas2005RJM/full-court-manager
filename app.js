...
function rosterTable(roster,featured){
  ...
  const configs={
    list:{heads:['NAME','POS','OVR','ROLLE','STATUS'],cells:p=>[`<strong>${p.name}</strong>`,p.position,`<b class="rating">${p.rating}</b>`,p.starterSlot?`Starter ${p.starterSlot}`:p.minutes>0?'Rotation':'Reserve',status(p)]},
    simple:{heads:['NAME','POS','ALTER','RATING','POT','FORM','MIN','VERTRAG','STATUS','PPG'],cells:p=>[`<strong>${p.name}</strong>`,p.position,p.age,`<b class="rating">${p.rating}${p.form>0?' ▲':''}</b>`,p.potential,`${p.form>0?'+':''}${p.form||0}`,p.minutes,`<span title="${contractLabel(p)}">${p.twoWay?'Two-Way':`${money(p.salary)} · ${p.years}J`}</span>`,status(p),perGame(p,'pts')]},
    details:{heads:['NAME','POS','OVR','POT','FORM','MORAL','FITNESS','TRAINING','ROLLE'],cells:p=>[`<strong>${p.name}</strong>`,p.position,`<b class="rating">${p.rating}</b>`,p.potential,`${p.form>0?'+':''}${p.form||0}`,p.morale||75,`${100-(p.fatigue||0)}%`,p.training||'Ausgewogen',p.starterSlot?`Starter ${p.starterSlot}`:p.minutes>0?`${p.minutes} Min.`:'Reserve']},
...
function updateTacticsLive(){
  const data=readTacticsForm();document.querySelectorAll('[data-tactic-value]').forEach(x=>x.textContent=data[x.dataset.tacticValue]);const impacts=$('#tactic-impacts');if(impacts)impacts.innerHTML=tacticImpacts(data).map(([type,text])=>`<span class="impact-${type==='+'?'plus':type==='-'?'minus':'neutral'}"><b>${type}</b>${text}</span>`).join('');const court=$('#court-system-name');if(court)court.textContent=data.system;const status=$('#tactic-save-status');if(status)status.textContent='UNGESPEICHERTE ÄNDERUNGEN';
}
...
$('#view-content').addEventListener('input',e=>{if(e.target.classList.contains('minute-input'))updateRotationLive();if(e.target.matches('[data-tactic]'))updateTacticsLive()});
...
$('#view-content').addEventListener('dragover',e=>{const row=e.target.closest('[data-bench-player]');if(!row||!draggedBenchPlayer||row===draggedBenchPlayer)return;e.preventDefault();const rect=row.getBoundingClientRect(),after=e.clientY>rect.top+rect.height/2;row.parentElement.insertBefore(draggedBenchPlayer,after?row.nextSibling:row);refreshBenchRanks()});
$('#view-content').addEventListener('drop',e=>{if(draggedBenchPlayer)e.preventDefault()});
