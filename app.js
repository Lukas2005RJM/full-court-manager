...
let scheduleMonth = 0;
let scheduleLocation = 'all';
let scheduleOpponent = 'all';
let standingsMode = 'conference';
let leagueMode = 'leaders';
let leagueLeaderStat = 'pts';
...
function normalizeSaveData(d){
  ...
  d.awardRankSnapshot=d.awardRankSnapshot||{};
  d.rotationPresets=d.rotationPresets||{};teams.forEach(t=>{d.rotationPresets[t.id]=d.rotationPresets[t.id]||Array(5).fill(null)});
  teams.forEach(t=>{const roster=d.rosters?.[t.id]||[],existing=new Set(roster.map(p=>p.name));fallbackRoster(t).filter(p=>!existing.has(p.name)).forEach(p=>{p.starterSlot=null;p.minutes=0;p.rotationOrder=roster.length;roster.push(p)});if(d.rosters)d.rosters[t.id]=normalizeRotationSetup(roster)});
  Object.values(d.rosters||{}).flat().forEach(p=>{p.form=p.form??0;p.morale=p.morale??75;p.injuryType=p.injuryType||'';p.tradeBlock=!!p.tradeBlock;p.ratingHistory=p.ratingHistory||[{season:d.seasonLabel||'2025/26',rating:p.rating}];p.potentialHistory=p.potentialHistory||[{season:d.seasonLabel||'2025/26',day:d.day||0,potential:p.potential}];p.potentialMomentum=p.potentialMomentum??0;p.seasonInjuryGames=p.seasonInjuryGames??0;p.trainingSessions=p.trainingSessions??0});
  return d;
}
...
function rosterTable(roster,featured){
  ...
  const configs={
    ...
    simple:{heads:['NAME','POS','ALTER','RATING','POT','ENTW.','MIN','VERTRAG','STATUS','PPG'],cells:p=>[`<strong>${p.name}</strong>`,p.position,p.age,`<b class="rating">${p.rating}${p.form>0?' ▲':''}</b>`,dynamicPotential(p),development(p),p.minutes,contract(p),status(p),perGame(p,'pts')]},
    details:{heads:['NAME','POS','OVR','POT','FORM','MORAL','FITNESS','TRAINING','ROLLE'],cells:p=>[`<strong>${p.name}</strong>`,p.position,`<b class="rating">${p.rating}</b>`,dynamicPotential(p),meter(50+(p.form||0)*10,'form'),meter(p.morale||75,'morale'),meter(100-(p.fatigue||0),'fitness'),p.training||'Ausgewogen',p.starterSlot?`Starter ${p.starterSlot}`:p.minutes>0?`${p.minutes} Min.`:'Reserve']},
...
function renderRotation(){
  ...
  const benchList=document.querySelector('.bench-drag-list'),benchTitle=document.querySelector('.bench-head h3');
  if(benchTitle)benchTitle.innerHTML=`BANKREIHENFOLGE <b>${bench.length}</b>`;
  if(benchList){benchList.className='bench-card-grid';benchList.style.cssText='display:block!important;position:relative!important;overflow:hidden!important;min-height:0!important;';[...benchList.children].forEach((row,index)=>{const column=index%2,line=Math.floor(index/2);row.className='bench-card';row.style.cssText=`display:grid!important;position:absolute!important;left:${column?'calc(50% + 4px)':'0'}!important;top:${line*51}px!important;width:calc(50% - 4px)!important;height:46px!important;min-height:46px!important;max-height:46px!important;margin:0!important;box-sizing:border-box!important;overflow:hidden!important;transform:none!important;`;const image=row.querySelector('.bench-photo');if(image)image.removeAttribute('style')})}
...
function refreshBenchRanks(){const rows=[...document.querySelectorAll('[data-bench-player]')];rows.forEach((x,i)=>{x.querySelector('.bench-rank').textContent=i+1;if(x.classList.contains('bench-card')){const column=i%2,line=Math.floor(i/2);x.style.left=column?'calc(50% + 4px)':'0';x.style.top=`${line*51}px`}const buttons=x.querySelectorAll('button');if(buttons.length>1){buttons[0].disabled=i===0;buttons[1].disabled=i===rows.length-1}});}
...
function leagueLeaders(stat,limit=10){return teams.flatMap(t=>save.rosters[t.id].map(p=>({p,t,value:stat==='min'?p.minutes:(p.stats.gp?p.stats[stat]/p.stats.gp:0)}))).filter(x=>stat==='min'||x.p.stats.gp).sort((a,b)=>b.value-a.value).slice(0,limit);}
...
function awardRaces(){const all=allLeaguePlayers().filter(x=>x.p.stats.gp),line=x=>({p:x.p,t:x.t,ppg:x.p.stats.pts/x.p.stats.gp,rpg:x.p.stats.reb/x.p.stats.gp,apg:x.p.stats.ast/x.p.stats.gp,spg:x.p.stats.stl/x.p.stats.gp,bpg:x.p.stats.blk/x.p.stats.gp});const rows=all.map(line),sort=fn=>rows.slice().sort((a,b)=>fn(b)-fn(a)).slice(0,5);return{MVP:sort(x=>x.ppg+x.apg*.7+x.rpg*.45+(x.p.rating-75)*.35),DPOY:sort(x=>x.spg*3+x.bpg*3+x.rpg*.35+x.p.rating*.25),ROY:rows.filter(x=>x.p.age<=21).sort((a,b)=>(b.ppg+b.apg*.5+b.rpg*.3)-(a.ppg+a.apg*.5+a.rpg*.3)).slice(0,5),MIP:sort(x=>(x.p.rating-(x.p.ratingHistory?.[0]?.rating??x.p.rating))*5+x.ppg),SIXTH:rows.filter(x=>!x.p.starterSlot&&x.p.minutes>0).sort((a,b)=>(b.ppg+b.apg*.5)-(a.ppg+a.apg*.5)).slice(0,5)}}
function allLeagueTeams(type){const players=allLeaguePlayers().filter(x=>x.p.stats.gp),score=(x,kind)=>kind==='defense'?(x.p.stats.stl+x.p.stats.blk)/x.p.stats.gp+x.p.rating/20:(x.p.stats.pts+x.p.stats.ast*.7+x.p.stats.reb*.35)/x.p.stats.gp+x.p.rating/25;return players.sort((a,b)=>score(b,type)-score(a,type)).slice(0,15)}
function renderLeague(){const modes=[['leaders','BESTENLISTEN'],['awards','AWARD RENNEN'],['teams','TEAMS & POWER'],['injuries','VERLETZUNGEN'],['history','HISTORIE']],tabs=`<div class="league-tabs">${modes.map(([id,label])=>`<button class="${leagueMode===id?'active':''}" data-league-mode="${id}">${label}</button>`).join('')}</div>`,leaderTable=(title,stat,suffix)=>`<div class="league-stat-card"><h3>${title}</h3>${leagueLeaders(stat,15).map((x,i)=>`<button data-league-player="${x.p.id}"><span>${i+1}</span>${imageMarkup(save.playerImages?.[x.p.id],'league-player-photo',x.p.name,`<i>${x.p.number}</i>`)}<strong>${x.p.name}<small>${x.t.abbr} · ${x.p.position}</small></strong><b>${x.value.toFixed(1)} ${suffix}</b></button>`).join('')||'<p class="muted">Noch keine Spiele absolviert.</p>'}</div>`;let content='';if(leagueMode==='leaders')content=`<div class="league-leader-grid">${leaderTable('PUNKTE','pts','PPG')}${leaderTable('REBOUNDS','reb','RPG')}${leaderTable('ASSISTS','ast','APG')}${leaderTable('STEALS','stl','SPG')}${leaderTable('BLOCKS','blk','BPG')}${leaderTable('MINUTEN','pts','')}</div>`;if(leagueMode==='awards'){const races=awardRaces();content=`<div class="award-race-grid">${Object.entries(races).map(([name,list])=>`<div class="award-race"><h3>${name==='SIXTH'?'SIXTH MAN':name}</h3>${list.map((x,i)=>`<button data-league-player="${x.p.id}"><span>${i+1}</span><strong>${x.p.name}<small>${x.t.abbr} · ${x.ppg.toFixed(1)} PTS · ${x.rpg.toFixed(1)} REB · ${x.apg.toFixed(1)} AST</small></strong><p>${i===0?'Führt durch Produktion, Rating und Teamrolle.':'Im Rennen durch starke Saisonwerte.'}</p></button>`).join('')||'<p class="muted">Noch keine Kandidaten.</p>'}</div>`).join('')}</div>`}if(leagueMode==='teams'){const power=[...teams].sort((a,b)=>teamPower(b.id)-teamPower(a.id));content=`<div class="team-power-grid"><div class="league-stat-card"><h3>POWER RANKING</h3>${power.map((t,i)=>`<button data-league-team="${t.id}"><span>${i+1}</span>${imageMarkup(save.teamImages?.[t.id],'standing-logo',fullName(t),`<i>${t.abbr}</i>`)}<strong>${fullName(t)}<small>${save.records[t.id].wins}-${save.records[t.id].losses}</small></strong><b>${teamPower(t.id).toFixed(1)}</b></button>`).join('')}</div><div class="league-stat-card"><h3>TEAMSTATISTIKEN</h3>${[...teams].sort((a,b)=>save.records[b.id].pf/Math.max(1,save.records[b.id].wins+save.records[b.id].losses)-save.records[a.id].pf/Math.max(1,save.records[a.id].wins+save.records[a.id].losses)).map((t,i)=>{const r=save.records[t.id],gp=Math.max(1,r.wins+r.losses);return`<button data-league-team="${t.id}"><span>${i+1}</span>${imageMarkup(save.teamImages?.[t.id],'standing-logo',fullName(t),`<i>${t.abbr}</i>`)}<strong>${fullName(t)}<small>${(r.pf/gp).toFixed(1)} PPG · ${(r.pa/gp).toFixed(1)} OPP</small></strong><b>${(r.pf-r.pa)>0?'+':''}${r.pf-r.pa}</b></button>`}).join('')}</div></div>`}if(leagueMode==='injuries'){const injured=allLeaguePlayers().filter(x=>x.p.injury>0).sort((a,b)=>b.p.injury-a.p.injury);content=`<div class="injury-board">${injured.map(x=>`<button data-league-player="${x.p.id}">${imageMarkup(save.playerImages?.[x.p.id],'league-player-photo',x.p.name,`<i>${x.p.number}</i>`)}<strong>${x.p.name}<small>${x.t.abbr} · ${x.p.position}</small></strong><span>${x.p.injuryType}<b>${x.p.injury} Spiele</b></span></button>`).join('')||'<div class="content-card empty-state"><h3>Keine Verletzungen</h3><p>Aktuell sind alle Spieler einsatzbereit.</p></div>'}</div>`}if(leagueMode==='history')content=`<div class="history-board"><div class="content-card"><h3>SAISONHISTORIE</h3>${save.history.slice().reverse().map(h=>`<div class="history-row"><b>${h.season}</b><span>Champion</span><strong>${fullName(teamById(h.champion))}</strong></div>`).join('')||'<p class="muted">Noch keine Saison abgeschlossen.</p>'}</div><div class="content-card"><h3>AWARD-HISTORIE</h3>${save.awards.slice().reverse().map(a=>`<div class="history-row"><b>${a.season}</b><span>MVP ${a.mvp}</span><span>DPOY ${a.dpoy}</span><span>ROY ${a.roy}</span></div>`).join('')||'<p class="muted">Noch keine Awards vergeben.</p>'}</div><div class="content-card"><h3>ALL-LEAGUE TEAMS</h3><p class="muted">Aktuelle Projektion</p>${['All-NBA', 'All-Defense','All-Rookie'].map((label,i)=>`<div class="projected-team"><b>${label}</b><span>${(i===0?allLeagueTeams('offense'):i===1?allLeagueTeams('defense'):allLeaguePlayers().filter(x=>x.p.age<=21).sort((a,b)=>b.p.rating-a.p.rating)).slice(0,5).map(x=>x.p.name).join(' · ')||'-'}</span></div>`).join('')}</div></div>`;$('#view-content').innerHTML=`${tabs}${content}`}

const leagueStatMeta={pts:['PUNKTE','PPG'],reb:['REBOUNDS','RPG'],ast:['ASSISTS','APG'],stl:['STEALS','SPG'],blk:['BLOCKS','BPG'],min:['MINUTEN','MPG']};
function leagueProjectionValue(p,stat){const pos=p.position;const big=['PF','C','FC'].includes(pos),guard=['PG','SG','G','GF'].includes(pos);return p.rating+(stat==='reb'?(big?8:-3):0)+(stat==='ast'?(guard?8:-4):0)+(stat==='blk'?(big?7:-4):0)+(stat==='stl'?((guard||pos==='SF')?4:0):0)+(stat==='min'?p.minutes/5:0)}
function leagueDashboardLeaders(stat){if(save.day>0)return leagueLeaders(stat,10);return allLeaguePlayers().map(x=>({...x,value:null,projection:leagueProjectionValue(x.p,stat)})).sort((a,b)=>b.projection-a.projection).slice(0,10)}
function playerSeasonLine(p){if(!p.stats.gp)return'PRESEASON-PROJEKTION';const avg=key=>(p.stats[key]/p.stats.gp).toFixed(1);return`${avg('pts')} PTS · ${avg('reb')} REB · ${avg('ast')} AST`}
function playerOfTheWeek(){const played=allLeaguePlayers().filter(x=>x.p.stats.gp);if(!played.length)return allLeaguePlayers().sort((a,b)=>b.p.rating-a.p.rating)[0];return played.sort((a,b)=>{const score=x=>x.p.stats.pts/x.p.stats.gp+(x.p.stats.reb/x.p.stats.gp)*.45+(x.p.stats.ast/x.p.stats.gp)*.65+(x.p.stats.stl/x.p.stats.gp)*1.8+(x.p.stats.blk/x.p.stats.gp)*1.8+(x.p.form||0)*.5;return score(b)-score(a)})[0]}
function leagueTabs(){const modes=[['leaders','BESTENLISTEN'],['awards','AWARD RENNEN'],['teams','TEAMS & POWER'],['injuries','VERLETZUNGEN'],['history','HISTORIE']];return`<div class="league-tabs league-dashboard-tabs">${modes.map(([id,label])=>`<button class="${leagueMode===id?'active':''}" data-league-mode="${id}">${label}</button>`).join('')}</div>`}
function teamRecordLabel(t){const r=save.records[t.id];return`${r.wins}-${r.losses}`}
function renderLeagueLeaderDashboard(){
  const leaders=leagueDashboardLeaders(leagueLeaderStat),meta=leagueStatMeta[leagueLeaderStat],weekly=playerOfTheWeek(),podium=leaders.slice(0,3),rest=leaders.slice(3);
  const value=x=>save.day>0?`${x.value.toFixed(1)} ${meta[1]}`:'PROJEKTION';
  $('#view-content').innerHTML=`${leagueTabs()}<div class="league-home-layout"><section class="league-feature-board"><div class="league-board-head"><div><p class="eyebrow">NBA STAT CENTER</p><h2>TOP 10 · ${meta[0]}</h2><small>${save.day>0?'Echte Saisonstatistiken':'Preseason-Projektion · ab Spieltag 1 automatisch echte Statistiken'}</small></div><div class="league-stat-picker">${Object.entries(leagueStatMeta).map(([id,[label]])=>`<button class="${leagueLeaderStat===id?'active':''}" data-leader-stat="${id}">${label}</button>`).join('')}</div></div><div class="league-podium">${podium.map((x,i)=>`<button class="podium-place place-${i+1}" data-league-player="${x.p.id}"><span class="podium-rank">${i+1}</span>${imageMarkup(save.playerImages?.[x.p.id],'podium-player',x.p.name,`<i>${x.p.number}</i>`)}${imageMarkup(save.teamImages?.[x.t.id],'podium-team',fullName(x.t),`<i>${x.t.abbr}</i>`)}<strong>${x.p.name}</strong><small>${fullName(x.t)} · ${x.p.position}</small><b>${value(x)}</b></button>`).join('')}</div><div class="league-top-ten">${rest.map((x,i)=>`<button data-league-player="${x.p.id}"><span>${i+4}</span>${imageMarkup(save.playerImages?.[x.p.id],'league-row-player',x.p.name,`<i>${x.p.number}</i>`)}${imageMarkup(save.teamImages?.[x.t.id],'league-row-team',fullName(x.t),`<i>${x.t.abbr}</i>`)}<strong>${x.p.name}<small>${fullName(x.t)} · ${x.p.position}</small></strong><b>${value(x)}</b></button>`).join('')}</div></section><aside class="player-week-card"><p class="eyebrow">SPIELER DER WOCHE</p><div class="player-week-visual">${imageMarkup(save.playerImages?.[weekly.p.id],'player-week-photo',weekly.p.name,`<i>${weekly.p.number}</i>`)}${imageMarkup(save.teamImages?.[weekly.t.id],'player-week-logo',fullName(weekly.t),`<i>${weekly.t.abbr}</i>`)}</div><h2>${weekly.p.name}</h2><strong>${fullName(weekly.t)} · ${weekly.p.position}</strong><p>${playerSeasonLine(weekly.p)}</p><div class="player-week-record"><span>TEAM-BILANZ<b>${teamRecordLabel(weekly.t)}</b></span><span>FORM<b>${weekly.p.form>0?'+':''}${weekly.p.form||0}</b></span></div><button data-league-player="${weekly.p.id}">SPIELERPROFIL ÖFFNEN</button></aside></div>`;
}
function awardScore(name,x){if(name==='MVP')return x.ppg+x.apg*.7+x.rpg*.45+x.spg*1.4+x.bpg*1.4;if(name==='DPOY')return x.spg*4+x.bpg*4+x.rpg*.45;if(name==='ROY')return x.ppg+x.apg*.55+x.rpg*.35;if(name==='MIP')return x.ppg+(x.p.rating-(x.p.ratingHistory?.[0]?.rating??x.p.rating))*4;if(name==='SIXTH')return x.ppg+x.apg*.55+x.rpg*.25;return 0}
function renderLeagueAwardDashboard(){
  const races=awardRaces(),labels={MVP:'MOST VALUABLE PLAYER',DPOY:'DEFENSIVE PLAYER',ROY:'ROOKIE OF THE YEAR',MIP:'MOST IMPROVED',SIXTH:'SIXTH MAN'};
  const cards=Object.entries(races).map(([name,list])=>{const scored=list.map(x=>({...x,score:awardScore(name,x)})),total=scored.reduce((s,x)=>s+Math.max(1,x.score),0);return`<section class="award-dashboard-card"><header><p>${name}</p><h3>${labels[name]}</h3></header>${scored.map((x,i)=>{const old=save.awardRankSnapshot?.[name]?.[x.p.id],change=old?old-(i+1):0,prob=Math.round(Math.max(1,x.score)/Math.max(1,total)*100),record=teamRecordLabel(x.t);return`<button data-league-player="${x.p.id}"><span class="award-rank">${i+1}</span>${imageMarkup(save.playerImages?.[x.p.id],'award-player-photo',x.p.name,`<i>${x.p.number}</i>`)}${imageMarkup(save.teamImages?.[x.t.id],'award-team-logo',fullName(x.t),`<i>${x.t.abbr}</i>`)}<strong>${x.p.name}<small>${x.ppg.toFixed(1)} PTS · ${x.rpg.toFixed(1)} REB · ${x.apg.toFixed(1)} AST · ${record}</small></strong><span class="award-trend ${change>0?'up':change<0?'down':''}">${change>0?'▲ '+change:change<0?'▼ '+Math.abs(change):'–'}</span><b>${prob}%<small>${x.score.toFixed(1)} PKT</small></b></button>`}).join('')||'<p class="muted award-empty">Nach dem ersten Spiel erscheinen die Kandidaten.</p>'}</section>`}).join('');
  $('#view-content').innerHTML=`${leagueTabs()}<div class="award-dashboard-intro"><div><p class="eyebrow">NBA AWARD WATCH</p><h2>FÜNF RENNEN. JEDE WOCHE NEU.</h2></div><p>Wahrscheinlichkeit, Saisonwerte, Team-Bilanz und Veränderung zur letzten Wochenaufnahme.</p></div><div class="award-dashboard-grid">${cards}</div>`;
}
function renderLeagueDashboard(){if(leagueMode==='leaders')return renderLeagueLeaderDashboard();if(leagueMode==='awards')return renderLeagueAwardDashboard();return renderLeague()}
...
function scheduleDate(day){const date=new Date(2025,9,21+day);return date}
function filteredScheduleGames(){return gamesForTeam(save.activeTeam).filter(g=>(scheduleLocation==='all'||(scheduleLocation==='home')===(g.home===save.activeTeam))&&(scheduleOpponent==='all'||(g.home===save.activeTeam?g.away:g.home)===scheduleOpponent))}
function scheduleGameCard(g,compact=false){const home=g.home===save.activeTeam,opp=teamById(home?g.away:g.home),record=save.records[opp.id],prev=gamesForTeam(save.activeTeam).find(x=>x.day===g.day-1),b2b=!!prev,date=scheduleDate(g.day),next=!g.played&&g.day===Math.min(...gamesForTeam(save.activeTeam).filter(x=>!x.played).map(x=>x.day));return`<button class="schedule-game ${g.played?'played':'upcoming'} ${next?'next-game-highlight':''}" ${g.played?`data-boxscore="${g.id}"`:''}><span class="schedule-day">${date.toLocaleDateString('de-DE',{day:'2-digit',month:'short'})}<b>${home?'HEIM':'AUSWÄRTS'}</b></span>${imageMarkup(save.teamImages?.[opp.id],'schedule-logo',fullName(opp),`<i>${opp.abbr}</i>`)}<strong>${compact?opp.abbr:fullName(opp)}<small>${record.wins}-${record.losses}${b2b?' · BACK-TO-BACK':''}</small></strong><span class="schedule-result">${g.played?`${g.homeScore}-${g.awayScore}`:`Spieltag ${g.day+1}`}${g.quarters?`<small>${g.quarters.home.map((x,i)=>`Q${i+1} ${x}:${g.quarters.away[i]}`).join(' · ')}</small>`:''}</span></button>`}
function renderSchedule(){const all=filteredScheduleGames(),months=[...new Set(gamesForTeam(save.activeTeam).map(g=>scheduleDate(g.day).toLocaleDateString('de-DE',{month:'long',year:'numeric'})))],monthName=months[scheduleMonth]||months[0],monthGames=all.filter(g=>scheduleDate(g.day).toLocaleDateString('de-DE',{month:'long',year:'numeric'})===monthName),controls=`<div class="schedule-toolbar"><div><button class="${scheduleMode==='calendar'?'active':''}" data-schedule-mode="calendar">KALENDER</button><button class="${scheduleMode==='list'?'active':''}" data-schedule-mode="list">LISTE</button></div><select id="schedule-location"><option value="all">Alle Spiele</option><option value="home" ${scheduleLocation==='home'?'selected':''}>Heimspiele</option><option value="away" ${scheduleLocation==='away'?'selected':''}>Auswärtsspiele</option></select><select id="schedule-opponent"><option value="all">Alle Gegner</option>${teams.filter(t=>t.id!==save.activeTeam).map(t=>`<option value="${t.id}" ${scheduleOpponent===t.id?'selected':''}>${fullName(t)}</option>`).join('')}</select></div>`;if(scheduleMode==='list'){$('#view-content').innerHTML=`${controls}<div class="schedule-list content-card"><div class="card-head"><h3>82-SPIELE-SPIELPLAN</h3><span>${all.filter(g=>g.played).length}/${all.length}</span></div>${all.map(g=>scheduleGameCard(g)).join('')||'<p class="empty-copy">Keine Spiele für diesen Filter.</p>'}</div>`;return}const first=monthGames[0]?scheduleDate(monthGames[0].day):new Date(),offset=(first.getDay()+6)%7;$('#view-content').innerHTML=`${controls}<div class="calendar-card"><div class="calendar-head"><button data-calendar-move="-1" ${scheduleMonth<=0?'disabled':''}>‹</button><h2>${monthName}</h2><button data-calendar-move="1" ${scheduleMonth>=months.length-1?'disabled':''}>›</button></div><div class="calendar-week"><span>MO</span><span>DI</span><span>MI</span><span>DO</span><span>FR</span><span>SA</span><span>SO</span></div><div class="calendar-grid">${Array(offset).fill('<div class="calendar-empty"></div>').join('')}${monthGames.map(g=>`<div class="calendar-day"><b>${scheduleDate(g.day).getDate()}</b>${scheduleGameCard(g,true)}</div>`).join('')}</div></div>`}

const divisions={atl:'Southeast',bos:'Atlantic',bkn:'Atlantic',cha:'Southeast',chi:'Central',cle:'Central',det:'Central',ind:'Central',mia:'Southeast',mil:'Central',nyk:'Atlantic',orl:'Southeast',phi:'Atlantic',tor:'Atlantic',was:'Southeast',dal:'Southwest',den:'Northwest',gsw:'Pacific',hou:'Southwest',lac:'Pacific',lal:'Pacific',mem:'Southwest',min:'Northwest',nop:'Southwest',okc:'Northwest',phx:'Pacific',por:'Northwest',sac:'Pacific',sas:'Southwest',uta:'Northwest'};
...
function teamStandingData(t){const games=gamesForTeam(t.id).filter(g=>g.played),r=save.records[t.id],result=g=>(g.home===t.id?g.homeScore:g.awayScore)>(g.home===t.id?g.awayScore:g.homeScore),last=games.slice(-10),home=games.filter(g=>g.home===t.id),away=games.filter(g=>g.away===t.id),record=list=>`${list.filter(result).length}-${list.filter(g=>!result(g)).length}`,streak=games.length?(()=>{const win=result(games.at(-1)),count=games.slice().reverse().findIndex(g=>result(g)!==win);return`${win?'W':'L'}${count<0?games.length:count}`})():'-';return{...r,gp:r.wins+r.losses,pct:r.wins/Math.max(1,r.wins+r.losses),diff:r.pf-r.pa,last:last.map(g=>result(g)?'W':'L'),home:record(home),away:record(away),streak}}
function standingsRows(list,zone=true,startRank=0,reference=list){const leader=teamStandingData(reference[0]),active=save.activeTeam;return list.map((t,i)=>{const d=teamStandingData(t),rank=startRank+i,gb=((leader.wins-d.wins)+(d.losses-leader.losses))/2,status=zone?(rank<6?'playoff':rank<10?'playin':'lottery'):'';return`<button class="standings-table-row ${status} ${t.id===active?'my-team':''}" data-league-team="${t.id}"><span>${rank+1}</span>${imageMarkup(save.teamImages?.[t.id],'standing-logo',fullName(t),`<i>${t.abbr}</i>`)}<strong>${fullName(t)}<small>${zone?(rank<6?'PLAYOFFS':rank<10?'PLAY-IN':'LOTTERY'):divisions[t.id]}</small></strong><b>${d.wins}</b><b>${d.losses}</b><b>${d.pct.toFixed(3).replace('0.','.')}</b><b>${rank===0?'-':gb.toFixed(1)}</b><b class="${d.diff>=0?'positive':'negative'}">${d.diff>0?'+':''}${d.diff}</b><span class="form-dots">${d.last.map(x=>`<i class="${x==='W'?'win':'loss'}">${x}</i>`).join('')}</span><b>${d.home}</b><b>${d.away}</b><b>${d.streak}</b></button>`}).join('')}
function renderStandings(){const tabs=[['conference','CONFERENCE'],['division','DIVISION'],['league','GESAMTLIGA']],head=`<div class="standings-tabs">${tabs.map(([id,label])=>`<button class="${standingsMode===id?'active':''}" data-standings-mode="${id}">${label}</button>`).join('')}</div>`,table=(title,list,zone=true,startRank=0,reference=list)=>`<div class="standings-table"><h3>${title}</h3><div class="standings-table-head"><span>#</span><span></span><strong>TEAM</strong><b>W</b><b>L</b><b>PCT</b><b>GB</b><b>DIFF</b><span>L10</span><b>HEIM</b><b>AUSW.</b><b>SERIE</b></div>${standingsRows(list,zone,startRank,reference)}</div>`;let body='';if(standingsMode==='conference')body=`<div class="standings-conferences">${table('EASTERN CONFERENCE',conferenceStandings('East'))}${table('WESTERN CONFERENCE',conferenceStandings('West'))}</div>`;if(standingsMode==='division')body=`<div class="division-grid">${[...new Set(Object.values(divisions))].map(name=>table(name.toUpperCase(),teams.filter(t=>divisions[t.id]===name).sort((a,b)=>teamStandingData(b).pct-teamStandingData(a).pct),false)).join('')}</div>`;if(standingsMode==='league'){const all=[...teams].sort((a,b)=>teamStandingData(b).pct-teamStandingData(a).pct);body=`<div class="overall-standings-grid">${table('PLÄTZE 1–15',all.slice(0,15),false,0,all)}${table('PLÄTZE 16–30',all.slice(15),false,15,all)}</div>`}$('#view-content').innerHTML=head+body}
...
function developmentData(p){p.potentialHistory=p.potentialHistory||[{season:save.seasonLabel,day:save.day,potential:p.potential}];p.potentialMomentum=p.potentialMomentum??0;p.seasonInjuryGames=p.seasonInjuryGames??0;p.trainingSessions=p.trainingSessions??0;return p}
function potentialEvaluation(p){
  developmentData(p);const gp=Math.max(1,p.stats.gp),avg=key=>p.stats[key]/gp,impact=avg('pts')+avg('reb')*.45+avg('ast')*.65+(avg('stl')+avg('blk'))*1.5,expected=Math.max(5,(p.rating-60)*.9),performance=clamp((impact-expected)/8,-1.5,1.5);
  const age=p.age<=21?1.15:p.age<=24?.7:p.age<=27?.25:p.age<=30?0:p.age<=33?-.45:-.85,role=p.minutes>=28?.45:p.minutes>=18?.2:p.minutes<8?-.65:-.1,morale=(p.morale||75)>=85?.25:(p.morale||75)<60?-.3:0,form=(p.form||0)*.08,training=Math.min(.35,(p.trainingSessions||0)*.05),injuries=-Math.min(1,(p.seasonInjuryGames||0)/18);
  return age+performance+role+morale+form+training+injuries;
}
function updateDynamicPotential(p,offseason=false){
  developmentData(p);p.potentialMomentum+=potentialEvaluation(p)*(offseason?1.5:1);let delta=0;
  const upThreshold=p.age<=24?6.5:8.5,downThreshold=p.age>=32?-5.5:-8;
  if(p.potentialMomentum>=upThreshold&&p.age<=29){delta=1;p.potentialMomentum-=upThreshold}
  else if(p.potentialMomentum<=downThreshold){delta=-1;p.potentialMomentum-=downThreshold}
  if(offseason&&p.age>=34&&p.seasonInjuryGames>=18)delta=Math.min(delta,-1);
  if(!delta)return 0;const floor=p.age<=29?p.rating:Math.min(p.rating,p.potential),next=clamp(p.potential+delta,Math.max(55,floor),99);delta=next-p.potential;if(!delta)return 0;p.potential=next;p.potentialHistory.push({season:save.seasonLabel,day:save.day,potential:p.potential});return delta;
}
function updateLeaguePotential(){teams.forEach(t=>save.rosters[t.id].forEach(p=>{const delta=updateDynamicPotential(p);if(delta&&t.id===save.activeTeam)save.news.push(`${p.name}: Potenzial ${delta>0?'gestiegen':'gesunken'} auf ${p.potential}.`)}))}
function recoverPlayers(){teams.forEach(t=>save.rosters[t.id].forEach(p=>{developmentData(p);p.fatigue=clamp(p.fatigue-3,0,100);if(p.injury>0){p.seasonInjuryGames++;p.injury--;if(!p.injury)p.injuryType=''}if(Math.random()<.08)p.form+=(p.form>0?-1:p.form<0?1:0)}));}
function snapshotAwardRanks(){const races=awardRaces();save.awardRankSnapshot=Object.fromEntries(Object.entries(races).map(([name,list])=>[name,Object.fromEntries(list.map((x,i)=>[x.p.id,i+1]))]))}
function simulateDay(){if(save.day>=82){setupPostseason();return;}const games=save.schedule[save.day];games.forEach(g=>simulateGame(g));const mine=games.find(g=>g.home===save.activeTeam||g.away===save.activeTeam);save.lastGame=mine?.id||save.lastGame;save.day++;recoverPlayers();if(save.day%7===0){teams.forEach(t=>save.trainingPoints[t.id]+=3);snapshotAwardRanks()}if(save.day%14===0)updateLeaguePotential();if(mine)save.news.push(`${teamById(mine.home).abbr} ${mine.homeScore}:${mine.awayScore} ${teamById(mine.away).abbr}`);if(save.day>=82)setupPostseason();persist();}
function simulateDays(n){for(let i=0;i<n&&save.phase==='Regular Season';i++)simulateDay();renderView();}
function calculateAwards(){const races=awardRaces(),ranked=awardLeaders(),rookies=teams.flatMap(t=>save.rosters[t.id].filter(p=>p.age<=21).map(p=>({p,t,score:p.rating+(p.stats.gp?p.stats.pts/p.stats.gp:0)}))).sort((a,b)=>b.score-a.score),defenders=teams.flatMap(t=>save.rosters[t.id].map(p=>({p,t,score:p.rating+(p.stats.gp?(p.stats.stl+p.stats.blk)/p.stats.gp:0)*4}))).sort((a,b)=>b.score-a.score);const award={season:save.seasonLabel,mvp:ranked[0]?.p.name||'-',dpoy:defenders[0]?.p.name||'-',roy:rookies[0]?.p.name||'-',mip:races.MIP[0]?.p.name||'-',sixth:races.SIXTH[0]?.p.name||'-',allNba:allLeagueTeams('offense').slice(0,5).map(x=>x.p.name),allDefense:allLeagueTeams('defense').slice(0,5).map(x=>x.p.name),allRookie:allLeaguePlayers().filter(x=>x.p.age<=21).sort((a,b)=>b.p.rating-a.p.rating).slice(0,5).map(x=>x.p.name)};save.awards.push(award);save.leagueNews.push(`Awards: ${award.mvp} gewinnt MVP, ${award.dpoy} wird DPOY.`)}
...
function offseasonRatingChange(p){
  updateDynamicPotential(p,true);const gap=p.potential-p.rating,healthy=(p.seasonInjuryGames||0)<18,strongSeason=potentialEvaluation(p)>.65;let change=0;
  if(p.age<=21&&gap>0)change=rand(1,Math.min(3,gap));
  else if(p.age<=24&&gap>0)change=Math.random()<(strongSeason?.9:.68)?Math.min(rand(1,2),gap):0;
  else if(p.age<=27&&gap>0)change=Math.random()<(strongSeason?.62:.32)?1:0;
  else if(p.age<=30)change=gap>0&&strongSeason&&Math.random()<.3?1:0;
  else if(p.age<=33)change=Math.random()<(healthy?.42:.65)?-1:0;
  else change=-rand(1,(healthy?1:2));
  if(!healthy&&p.age>=29)change=Math.min(change,-1);return change;
}
function advanceSeason(){
  ...
  teams.forEach(t=>{
    ...
    save.rosters[t.id].forEach(p=>{
      const growth=offseasonRatingChange(p);p.age++;p.rating=clamp(p.rating+growth,55,99);if(p.age<=29)p.potential=Math.max(p.rating,p.potential);p.ratingHistory=p.ratingHistory||[];p.ratingHistory.push({season:save.seasonLabel,rating:p.rating});p.potentialHistory=p.potentialHistory||[];p.potentialHistory.push({season:save.seasonLabel,day:82,potential:p.potential});p.fatigue=0;p.injury=0;p.stats=resetStats();p.seasonInjuryGames=0;p.trainingSessions=0;
...
function openTeamSummary(id){const t=teamById(id),d=teamStandingData(t),roster=save.rosters[id].slice().sort((a,b)=>b.rating-a.rating),next=findNextGame(id);$('#modal-content').innerHTML=`<div class="team-summary-modal"><div class="team-summary-head">${imageMarkup(save.teamImages?.[id],'team-summary-logo',fullName(t),`<i>${t.abbr}</i>`)}<div><p class="eyebrow">${t.conference} · ${divisions[id]}</p><h2>${fullName(t)}</h2><strong>${d.wins}-${d.losses} · ${d.pct.toFixed(3).replace('0.','.')} · ${d.diff>0?'+':''}${d.diff}</strong></div></div><div class="team-summary-stats"><span>FORM<b>${d.last.join(' ')||'-'}</b></span><span>HEIM<b>${d.home}</b></span><span>AUSWÄRTS<b>${d.away}</b></span><span>SERIE<b>${d.streak}</b></span></div><h3>TOPSPIELER</h3><div class="team-summary-players">${roster.slice(0,5).map(p=>`<button data-league-player="${p.id}">${imageMarkup(save.playerImages?.[p.id],'league-player-photo',p.name,`<i>${p.number}</i>`)}<strong>${p.name}<small>${p.position} · OVR ${p.rating}</small></strong></button>`).join('')}</div>${next?`<p class="team-summary-next">Nächstes Spiel: Spieltag ${next.day+1} · ${teamById(next.home===id?next.away:next.home).abbr}</p>`:''}</div>`;$('#game-modal').classList.remove('hidden')}
...
document.addEventListener('click',e=>{
  ...
  const leaguePlayer=e.target.closest('[data-league-player]');if(leaguePlayer)return openPlayerProfile(leaguePlayer.dataset.leaguePlayer);
  const leagueTeam=e.target.closest('[data-league-team]');if(leagueTeam)return openTeamSummary(leagueTeam.dataset.leagueTeam);
  const scheduleModeButton=e.target.closest('[data-schedule-mode]');if(scheduleModeButton){scheduleMode=scheduleModeButton.dataset.scheduleMode;return renderSchedule()}
  const calendarMove=e.target.closest('[data-calendar-move]');if(calendarMove){scheduleMonth=Math.max(0,scheduleMonth+Number(calendarMove.dataset.calendarMove));return renderSchedule()}
  const standingsTab=e.target.closest('[data-standings-mode]');if(standingsTab){standingsMode=standingsTab.dataset.standingsMode;return renderStandings()}
  const leagueTab=e.target.closest('[data-league-mode]');if(leagueTab){leagueMode=leagueTab.dataset.leagueMode;return renderLeagueDashboard()}
  const leaderStat=e.target.closest('[data-leader-stat]');if(leaderStat){leagueLeaderStat=leaderStat.dataset.leaderStat;return renderLeagueLeaderDashboard()}
  ...
  const train=e.target.closest('[data-train]');if(train){const t=activeTeam();if(save.trainingPoints[t.id]<1)return alert('Keine Trainingspunkte verfügbar.');const p=save.rosters[t.id].find(x=>x.id===train.dataset.train),type=document.querySelector(`[data-training="${p.id}"]`).value,intense=['Wurf','Defense','Athletik','Spielverständnis'].includes(type);developmentData(p);if(type==='Regeneration'){p.fatigue=clamp(p.fatigue-22,0,100);p.morale=clamp((p.morale||75)+2,35,100)}else{p.trainingSessions++;if(p.rating<p.potential&&Math.random()<(type==='Ausgewogen'?.45:.58))p.rating++;p.fatigue=clamp(p.fatigue+(intense?7:4),0,100);if(intense&&p.fatigue>80&&Math.random()<.08){p.injury=1;p.injuryType='Trainingsüberlastung'}}p.training=type;save.trainingPoints[t.id]--;persist();return renderView('rotation')}
...
$('#view-content').addEventListener('change',e=>{if(e.target.id==='schedule-location'){scheduleLocation=e.target.value;renderSchedule()}if(e.target.id==='schedule-opponent'){scheduleOpponent=e.target.value;renderSchedule()}});
