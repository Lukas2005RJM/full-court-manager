...
function standingsRows(list,zone=true){const leader=teamStandingData(list[0]),active=save.activeTeam;return list.map((t,i)=>{const d=teamStandingData(t),gb=((leader.wins-d.wins)+(d.losses-leader.losses))/2,status=zone?(i<6?'playoff':i<10?'playin':'lottery'):'';return`<button class="standings-table-row ${status} ${t.id===active?'my-team':''}" data-league-team="${t.id}"><span>${i+1}</span>${imageMarkup(save.teamImages?.[t.id],'standing-logo',fullName(t),`<i>${t.abbr}</i>`)}<strong>${fullName(t)}<small>${zone?(i<6?'PLAYOFFS':i<10?'PLAY-IN':'LOTTERY'):divisions[t.id]}</small></strong><b>${d.wins}</b><b>${d.losses}</b><b>${d.pct.toFixed(3).replace('0.','.')}</b><b>${i===0?'-':gb.toFixed(1)}</b><b class="${d.diff>=0?'positive':'negative'}">${d.diff>0?'+':''}${d.diff}</b><span class="form-dots">${d.last.map(x=>`<i class="${x==='W'?'win':'loss'}">${x}</i>`).join('')}</span><b>${d.home}</b><b>${d.away}</b><b>${d.streak}</b></button>`}).join('')}
function renderStandings(){const tabs=[['conference','CONFERENCE'],['division','DIVISION'],['league','GESAMTLIGA']],head=`<div class="standings-tabs">${tabs.map(([id,label])=>`<button class="${standingsMode===id?'active':''}" data-standings-mode="${id}">${label}</button>`).join('')}</div>`,table=(title,list,zone=true)=>`<div class="standings-table"><h3>${title}</h3><div class="standings-table-head"><span>#</span><span></span><strong>TEAM</strong><b>W</b><b>L</b><b>PCT</b><b>GB</b><b>DIFF</b><span>L10</span><b>HEIM</b><b>AUSW.</b><b>SERIE</b></div>${standingsRows(list,zone)}</div>`;let body='';if(standingsMode==='conference')body=`<div class="standings-conferences">${table('EASTERN CONFERENCE',conferenceStandings('East'))}${table('WESTERN CONFERENCE',conferenceStandings('West'))}</div>`;if(standingsMode==='division')body=`<div class="division-grid">${[...new Set(Object.values(divisions))].map(name=>table(name.toUpperCase(),teams.filter(t=>divisions[t.id]===name).sort((a,b)=>teamStandingData(b).pct-teamStandingData(a).pct),false)).join('')}</div>`;if(standingsMode==='league')body=table('NBA GESAMTTABELLE',[...teams].sort((a,b)=>teamStandingData(b).pct-teamStandingData(a).pct),false);$('#view-content').innerHTML=head+body}
